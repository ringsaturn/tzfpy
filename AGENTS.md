...
- `src/lib.rs` contains the PyO3 extension module and the exported Python-callable functions. The mutually exclusive `lite` (default) and `full` Cargo features pick which tzf-dist dataset is embedded; the Python API is identical either way.
...
- `make build-full` builds the full-precision `+full` wheel (`--no-default-features --features full`). It is GitHub-Release-only; never publish it to PyPI.
